<template>
  <div>
    ...
    <node-panel v-show="cellType == 'node'" :cell="cell" :form-model="formModel">
      <detail-form type="node" :read-only="readOnly" />
      <slot name="node"></slot>
    </node-panel>
    <edge-panel v-show="cellType == 'edge'" :cell="cell" :form-model="formModel">
    ...
    this.$nextTick(() => {
      ...
      graph.on('cell:click', ({ cell }) => {
        if (cell.isNode()) {
          ...
          this.cellType = 'edge'
          let targetMName = cell.getAttrs().targetMarker
            ? cell.getAttrs().targetMarker.name
              ? cell.getAttrs().targetMarker.name
              : cell.getAttrs().targetMarker
            : 'classic'
          let sourceMName = cell.getAttrs().sourceMarker
            ? cell.getAttrs().sourceMarker.name
              ? cell.getAttrs().sourceMarker.name
              : cell.getAttrs().sourceMarker
            : 'path'
          this.formModel = {
